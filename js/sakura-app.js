...
function add_copyright () {
  ...
  function setClipboardText (event) {
    ...
    if (event.clipboardData) {
      ...
      event.clipboardData.setData('text/html', window.getSelection().toString().replace(/\r\n/g, '<br>'))
      event.clipboardData.setData('text/plain', window.getSelection().toString().replace(/\r\n/g, '\n'))
      addComment.createButterbar('复制成功！<br>Copied to clipboard successfully!', 1000)
    } else if (window.clipboardData) {
      // return window.clipboardData.setData('text', textData)
      return window.clipboardData.setData('text', window.getSelection().toString().replace(/\r\n/g, '\n'))
    }
  }
}
add_copyright()
